...
- Season separation: rec.season (= GetCurrentArenaSeason() stamped at write) IS the season key — no reset detection needed. AI.GetCurrentSeasonId() (live API, falls back to newest rec.season) / AI.HasMultipleSeasons() (matches span >1 season → show toggle). Aggregators take a seasonId filter (rec.season == seasonId; nil = all). Legacy matches with rec.season == nil count as "other season"
