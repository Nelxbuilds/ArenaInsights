...
- Season rollover detection (CapturePvPStats): GetPersonalRatedInfo's seasonPlayed is monotonic within a season and resets to 0 at a new season, so a DECREASE appends a boundary timestamp to ArenaInsightsDB.seasonBoundaries. Baselines tracked PER CHARACTER in ArenaInsightsDB.seasonState[charKey] = { max, seasonIndex } — a shared baseline would false-trigger on alt switches (seasonPlayed is per-character); a boundary is appended only when the character was in the latest season (alts catching up after a rollover add no second boundary). Runs regardless of the rating>0 guard. Independent of GetCurrentArenaSeason (unverified at boundaries). WARNING: reads seasonPlayed from the deprecated GetPersonalRatedInfo (see docs/bugs.md) — migrate both together
...
- Season separation: AI.GetSeasonBoundaries() / AI.GetSeasonCount() / AI.GetSeasonIndex(ts) — a match's 1-based season is derived from its timestamp vs ArenaInsightsDB.seasonBoundaries (set in Core.lua). Migration-free: existing records slot in by timestamp. Season 1 = before the first boundary; current season = at/after the last
